...
from game_envs import cart_pendulum_upright
...
def params_to_filename(params):
    return str(str(params['nn'][0])) + '-' + str(params['nn'][1]) + '-' + \
...
def log_results(filename, loss_log):
    ...
    with open('../results/single_agent_upright/loss_data-' + filename + '.csv', 'w') as lf:
        wr = csv.writer(lf)
...
def train_net(q_network, target_network, params, headless = False):
    ...
    game_state = cart_pendulum_upright.GameInstance(headless)
    ...
    #     # If we're done observing, start training.
        ...
        if t % int(train_frames/4) == 0:
            try:
                q_network.save_weights('../models/single-agent-upright-saved-models/' + filename + '-' +
                                str(t) + '.h5',
                                overwrite=True)
